...
//checks validity of split
    if (this.isEmpty()){
      ...
      this.displayAll();
      //if 1 item first list keeps it while second list stays 0
      NL.head = this.head.getNext();
      ...
      int count = 0;
      Node<T> temp = this.head;
      //finds the spot given then splits 
      ...
      this.displayAll();
      NL.head = temp.getNext();
      temp.setNext(null);

      NL.size = this.size - (index + 1);
      this.size = index + 1;
  ...
  public void insertSorted(T item){
    ...
    //if at front or comparable it inserts there
    if(this.head == null || ((Comparable<T>) item).compareTo(this.head.getElement()) <= 0){
      insertNode.setNext(this.head);
      this.head = insertNode;
    }else{
      //checks all values and inserts when applicable
      Node<T> temp = this.head;
  ...
  public void InsertionSort(){
    if(this.isEmpty()){
      System.out.println("an empty list cannot be sorted!");
    }
    else if (1 < size){
      //rounds up so that the bottom half is larger
      int split = (int) Math.ceil(this.size / 2.0) - 1;
      //does first split
      List<T> NL = this.splitAt(split);
      //continues to break down in order to solve recursively
      this.InsertionSort();
      NL.InsertionSort();
      //reinserts into list
      Node<T> current = NL.head;
      while (current != null) {
        this.insertSorted(current.getElement());
...
public class assignment5 {
  ...
  public static void main(String[] args) { // The main() method
    ...
    do{
      ...
      System.out.println( "11: insert item Sorted");
      System.out.println( "88: Split List");
      System.out.println( "99: insert sort list");
      ...
      switch (ch) {
        ...
        case 88:
          System.out.println( "enter index:");
          index = sc.nextInt();
          list.splitAt(index);   //did not add value because it is not needed for cases here and java cleans it up as no refrence to the object created. 
          break;
          case 99:
          list.InsertionSort();
          break; 
